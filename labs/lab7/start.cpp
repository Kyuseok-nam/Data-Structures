...
void print_grid(vector<vector<bool> > const& blocked_grid, int start_x, int start_y, std::vector<Point> path) {
    ...
    for (int y = 0; y < blocked_grid[0].size(); ++y) {
        for (int x = 0; x < blocked_grid.size(); ++x) {
            if (x == start_x && y == start_y)
                ...
                cout << " X";
            else if(counter < path.size() && path[counter].x == x && path[counter].y == y){
                cout << " $";
                counter++;
            } else
                cout << " .";
...
// ----------------------------------------------------------
int find_paths(int start_x, int start_y, vector<vector<bool> > const& blocked_grid, std::vector<std::vector<Point> >  &paths, int& counter){
    int numpaths = 0;
    ...
    if (start_x>0 && !blocked_grid[start_x][start_y] ) {
        Point one(start_x, start_y);
        paths[counter].push_back(one);
        numpaths += find_paths(start_x-1,start_y,blocked_grid, paths, counter);
    }
    if (start_y>0 && !blocked_grid[start_x][start_y]) {
        Point one(start_x, start_y);
        paths[counter].push_back(one);
        numpaths += find_paths(start_x,start_y-1,blocked_grid, paths, counter);
    }
...
int main(int argc, char* argv[]) {
    ...
    std::vector<Point> s;
    print_grid(blocked_grid, start_x, start_y, s);
    // Start here with your code...
    std::vector<std::vector<Point> > v;
    std::vector<Point> jj;
    v.push_back(jj);
    int counter = 0;
    int numpaths = find_paths(start_x, start_y, blocked_grid, v, counter);
    std::vector<Point> randomPath = v[0];
    std::cout << "number of possible paths is: " << numpaths << '\n';
    print_grid(blocked_grid, start_x, start_y, s);
    return 0;
}
